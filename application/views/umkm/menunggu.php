...
<div class="card">
        ...
        <table class="table table-bordered" id="dataTable">
                ...
                <tbody>
                    <?php foreach ($umkms as $umkm): ?>
                        <?php if ($umkm && isset($umkm->photo) && $umkm->status == 'menunggu'): ?>
                        <tr>
                            <td>
                                <img src="<?= base_url('uploads/umkm/' . $umkm->photo) ?>" width="50" height="50" alt="UMKM Logo" class="rounded-circle">
                            </td>
                            <td><?= $umkm->nama_pemilik ?></td>
                            <td style="white-space: pre-line; max-width: 150px; overflow: hidden;"><?= $umkm->umkm_address ?></td>
                            <td><?= $umkm->nomor_hp ?></td>
                            <td><?= $umkm->email ?></td>
                            <td><span class="badge bg-label-warning"><?= $umkm->status ?></span></td>
                            <td>
                            <div class="dropdown">
    ...
    <div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
        <button class="dropdown-item btn btn-edit" data-toggle="modal" data-target="#editModal" data-umkmid="<?= $umkm->id ?>" data-umkmname="<?= $umkm->nama_pemilik ?>" data-umkmaddress="<?= $umkm->umkm_address ?>" data-umkmphone="<?= $umkm->nomor_hp ?>" data-status="<?= $umkm->status ?>" data-email="<?= $umkm->email ?>" data-photo="<?= $umkm->photo ?>">
            ...
            <div class="modal-body">
                <form id="addUmkmForm" method="POST" action="<?php echo site_url('umkm/insert_umkm'); ?>" enctype="multipart/form-data">
                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-select" name="username" id="username" required aria-label="Default select example">
    ...
    <div class="form-group">
    <label for="umkm_address" class="col-form-label">Alamat</label>
    <textarea class="form-control" name="umkm_address" id="umkm_address" required></textarea>
</div>
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                <input type="hidden" name="username" id="edit_username" value="<?php echo $umkm->username; ?>">
                    <div class="form-group">
                        <label for="edit_nama_pemilik" class="col-form-label">Nama Pemilik</label>
                        <input
                          type="text"
                          class="form-control"
                        name="nama_pemilik" id="edit_nama_pemilik"
...
.swal2-container {
    ...
    var umkm_address = $('#umkm_address').val();
    var nomor_hp = $('#nomor_hp').val();
    var email = $('#email').val();
    var status = $('#status').val();

    if (nama_pemilik.trim() === '' || username.trim() === '' || umkm_address.trim() === '' || nomor_hp.trim() === '' || email.trim() === '' || status.trim() === '') {
        Swal.fire({
            icon: 'error',
            title: 'Error',
        ...
        $.ajax({
            ...
            success: function (response) {
                if (response.message === 'UMKM added successfully.') {
                    Swal.fire({
                        icon: 'success',
                        title: 'Success',
